...
YoptaScript — экспериментальный язык со сленговым синтаксисом, который мы исполняем напрямую: tree-walking интерпретатором и стековой байткод-VM (генерации JavaScript нет), а семантика зеркалит современный ECMAScript. Этот документ фиксирует договорённости о том, какие лексемы и конструкции поддерживает язык. Формальная запись нужна, чтобы лексер, парсер и остальные фазы работали согласованно.
...
- Однострочные комментарии `// ...` продолжаются до конца строки.
...
- Diagnostic сообщает пользователю исходную позицию; те же span’ы использует форматтер для генерации source map в режиме `yps fmt --source-map`.
