...
mod suite {
    ...
    #[test]
    fn for_of_destructuring_patterns_round_trip() {
        let src = concat!(
            "го (гыы [а, б] сашаГрей пары) {\n    сказать(а);\n}\n",
            "го (гыы { х, у } сашаГрей точки) {\n    сказать(х);\n}\n",
            "го (гыы [первый = 0, ...хвост] сашаГрей сп) {\n    сказать(первый);\n}\n",
            "го (гыы { к: { л } } сашаГрей сп) {\n    сказать(л);\n}\n",
            "го (гыы [м, н] из об) {\n    сказать(м);\n}\n",
            "го сидетьНахуй (гыы [п, р] сашаГрей ист) {\n    сказать(п);\n}\n",
        );
        let out = parse_and_format(src);
        assert!(programs_equivalent_str(src, &out), "round-trip нарушен для деструктуризации в for-of/for-in: {out:?}");
        assert!(out.contains("го (гыы [а, б] сашаГрей пары)"), "форматтер должен печатать паттерн массива: {out:?}");
