...
fn invalid_assignment_targets_are_parse_errors() {
    for (src, expected) in [
        ("5 = 1;", "Недопустимая цель присваивания"),
        ("ф() = 1;", "Недопустимая цель присваивания"),
        ("1 || (1 = 2);", "Недопустимая цель присваивания"),
        ("а?.б = 1;", "Недопустимая цель присваивания"),
        ("[а, ф()] = [1, 2];", "Недопустимая цель присваивания"),
        ("({к: 5} = о);", "Недопустимая цель присваивания"),
        ("!х += 1;", "Недопустимая цель составного присваивания"),
        ("го (;;!х += 1) {}", "Недопустимая цель составного присваивания"),
        ("[а] += 1;", "Недопустимая цель составного присваивания"),
        ("1 || 1++;", "'++' / '--' можно применить только к переменной"),
        ("(х)++;", "'++' / '--' можно применить только к переменной"),
        ("о.а--;", "'++' / '--' можно применить только к переменной"),
    ] {
        let (_, diags) = parse_program_from_source(src);
        let messages = diag_messages(&diags);
        assert!(messages.iter().any(|m| m.contains(expected)), "исходник {src:?}: {messages:?}");
    }
}

#[test]
fn valid_assignment_targets_parse_cleanly() {
    for src in [
        "х = 1;",
        "(х) = 1;",
        "о.а = 1;",
